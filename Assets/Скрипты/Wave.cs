...
// Этот скрипт генерирует волны. Он определяет,как много врагов будут спавниться с их скоростью, здоровьем. 
// Так же он определяет их темп стрельбы и стиль движения.

[System.Serializable]
public class Shooting
{
    [Range(0,100)]
    [Tooltip("Шанс выстрела (0%-100%)")]
    public int shotChance;

    [Tooltip("Время после которого может быть сделан выстрел(мин и макс)")]
...
public class Wave : MonoBehaviour {
    ...
    IEnumerator CreateEnemyWave() //Генерирование волны на основе этих параметров
    {
        ...
        if (testMode)       //Если тест мод включен, тогда перереспавнивает эту волну через 3 сек
        {
    ...
    void DrawPath(Transform[] path) //Для рисования пути в эдиторе
    {
    ...
    Vector3[] CreatePoints(Vector3[] path)  //Создание точек для передвижения
    {
